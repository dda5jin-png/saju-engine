...
export default function ResultClientPage({ resultId }: Props) {
  ...
  const [darkReport, setDarkReport] = useState(false);
  ...
  return (
    <main className={`result-theme ${darkReport ? 'result-theme-dark' : 'result-theme-light'} min-h-screen p-5 pb-24 md:pt-10`}>
      <div className="result-report-font max-w-2xl mx-auto space-y-8 pb-8">
        <div data-share-exclude className="flex justify-end">
          <button
            type="button"
            aria-pressed={darkReport}
            aria-label="밝기 조정"
            onClick={() => setDarkReport((value) => !value)}
            className="brightness-toggle inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-black transition active:scale-95"
          >
            {darkReport ? <SunMedium size={15} /> : <Moon size={15} />}
            <span>밝기 조정</span>
            <span className="brightness-toggle-state">{darkReport ? '밝게 보기' : '다크로 보기'}</span>
          </button>
        </div>
