...
export default function LandingPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#0F172A] px-6 py-8 text-[#F8F4EA]">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_8%,rgba(214,180,106,0.16),transparent_34%),radial-gradient(circle_at_82%_72%,rgba(124,58,237,0.14),transparent_30%)]" />
      <HeaderAuthButton />
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl flex-col justify-center gap-10 text-center">
        <section className="space-y-7">
          ...
          <div className="space-y-5">
            <h1 className="mx-auto max-w-3xl text-4xl font-black leading-[1.17] tracking-tight md:text-6xl break-keep">
              당신의 사주를<br />
              <span className="text-[#D6B46A]">에너지 구조</span>로 해석합니다.
            </h1>
        ...
        <section className="grid gap-3 text-left md:grid-cols-4">
          {['사주 에너지 분석', '오행 균형', '보석 추천', '착용 가이드'].map((item, index) => (
            <div key={item} className="rounded-2xl border border-[#D6B46A]/18 bg-white/[0.035] p-4">
